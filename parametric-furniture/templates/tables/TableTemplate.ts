...
export class TableTemplate implements ParametricTemplate {
  ...
  private generateTableTop(parameters: ParametricParameters): THREE.Mesh {
  ...
  private generateTableLegs(parameters: ParametricParameters): THREE.Mesh[] {
    ...
    legConfiguration.positions.forEach((position, index) => {
      const legGeometry = this.createCulturalLegGeometry(
        parameters.culture, 
        legThickness, 
        legHeight,
        index,
        legConfiguration.style
      );
  ...
  private generateSupports(parameters: ParametricParameters): THREE.Mesh[] {
  ...
  private addCulturalTableElements(parameters: ParametricParameters): THREE.Object3D[] {
  ...
  private createJapaneseTableTop(width: number, depth: number, thickness: number): THREE.BufferGeometry {
  ...
  private createScandinavianTableTop(width: number, depth: number, thickness: number): THREE.BufferGeometry {
  ...
  private createItalianTableTop(width: number, depth: number, thickness: number): THREE.BufferGeometry {
    // Elegant proportions with sophisticated edge treatment
    const geometry = new THREE.BoxGeometry(width, thickness, depth);
    return geometry;
  }

  private createFrenchTableTop(width: number, depth: number, thickness: number): THREE.BufferGeometry {
    // Graceful, refined design with gentle curves
    const geometry = new THREE.BoxGeometry(width, thickness, depth);
    return geometry;
  }

  private createModernTableTop(width: number, depth: number, thickness: number): THREE.BufferGeometry {
    // Clean, geometric form
    const geometry = new THREE.BoxGeometry(width, thickness, depth);
    return geometry;
  }

  private determineLegConfiguration(parameters: ParametricParameters): LegConfiguration {
    switch (parameters.culture) {
      case 'japanese':
  ...
  private createTurnedLegGeometry(thickness: number, height: number): THREE.BufferGeometry {
  ...
  private createPedestalGeometry(thickness: number, height: number): THREE.BufferGeometry {
  ...
  private calculateFourLegPositions(parameters: ParametricParameters): Array<{x: number, z: number}> {
  ...
  private createGrainDetails(parameters: ParametricParameters): THREE.Object3D[] {
  ...
  private createMinimalDetails(parameters: ParametricParameters): THREE.Object3D[] {
    const details: THREE.Object3D[] = [];
  ...
  private createLuxuryDetails(parameters: ParametricParameters): THREE.Object3D[] {
    const details: THREE.Object3D[] = [];
  ...
  private getTableTypeName(type: any): string {
  ...
  private getCulturalSignificance(culture: CultureType, type: any): string {
