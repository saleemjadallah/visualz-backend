...
export class CulturalKnowledgeBase {
  private cultures: Map<CultureType, CulturalProfile> = new Map();
  ...
  getCulturalProportions(culture: CultureType, furnitureType: FurnitureType): CulturalProportions {
    ...
    if (!profile) throw new Error(`Unknown culture: ${culture}`);
  ...
  validateCulturalAuthenticity(
    ...
    if (!profile) return { overall: 0, proportions: 0, materials: 0, construction: 0, aesthetics: 0, culturalElements: 0 };
  ...
  private adaptProportionsForFurnitureType(
    baseProportions: CulturalProportions,
  ...
  private scoreCulturalProportions(parameters: ParametricParameters, profile: CulturalProfile): number {
    ...
    if (heightDifference > 0.1) score -= 20; // 10% tolerance
    
    // Check width/depth ratios
    const ratio = parameters.width / parameters.depth;
    if (ratio < 0.5 || ratio > 2.0) score -= 10; // Reasonable proportions
    
    return Math.max(0, Math.min(100, score));
  }

  private scoreCulturalMaterials(parameters: ParametricParameters, profile: CulturalProfile): number {
    let score = 0;
  ...
  private scoreCulturalAesthetics(parameters: ParametricParameters, profile: CulturalProfile): number {
    let score = 50; // Base score
    
    // Color palette alignment
    const colorMatch = parameters.colorPalette.some(color => 
      profile.aesthetics.colorPalette.includes(color)
    );
    ...
    if (intensityDiff < 0.2) score += 25;
    
    return Math.max(0, Math.min(100, score));
  }

  private scoreCulturalElements(parameters: ParametricParameters, profile: CulturalProfile): number {
    let score = 0;
    const totalElements = parameters.culturalElements.length;
    
    if (totalElements === 0) return 30; // Minimal but acceptable
    
    const matchingElements = parameters.culturalElements.filter(element =>
      profile.aesthetics.decorativeElements.includes(element)
    );
  ...
  private scoreCulturalConstruction(parameters: ParametricParameters, profile: CulturalProfile): number {
  ...
  private getExpectedDecorativeIntensity(culture: CultureType, formality: string): number {
    const baseIntensity: Record<CultureType, number> = {
      'japanese': 0.3,
      'scandinavian': 0.2,
      'italian': 0.8,
      'french': 0.6,
    ...
    const formalityMultiplier: Record<string, number> = {
      'casual': 0.8,
      'semi-formal': 1.0,
      'formal': 1.2,
      'ceremonial': 1.5
  ...
  isMaterialCulturallyAppropriate(material: MaterialType, culture: CultureType): boolean {
    const profile = this.getCulturalProfile(culture);
    if (!profile) return true; // Default to appropriate if unknown culture
