...
export class EnhancedChairTemplate {
    constructor() {
    ...
    convertToChairParameters(params) {
        return {
            ...params,
            ...
            upholsteryMaterial: params.secondaryMaterial?.startsWith('fabric') ?
                params.secondaryMaterial : 'none'
        };
    }
    validateAndAdjustParameters(params) {
    ...
    generateSeat(dimensions, params) {
        const culturalData = this.culturalData.get(params.culture);
        let seatGeometry;
        switch (culturalData.styleElements.seatStyle) {
            case 'flat':
                seatGeometry = new THREE.BoxGeometry(dimensions.seatWidth, dimensions.seatThickness, dimensions.seatDepth);
                break;
            case 'contoured':
    ...
    generateBackrest(dimensions, params) {
        const culturalData = this.culturalData.get(params.culture);
        let backrestGeometry;
        switch (culturalData.styleElements.backStyle) {
            case 'straight':
                backrestGeometry = new THREE.BoxGeometry(dimensions.seatWidth * 0.9, dimensions.backrestHeight, dimensions.backrestThickness);
                break;
            case 'curved':
    ...
    generateLegs(dimensions, params) {
        const culturalData = this.culturalData.get(params.culture);
        const legs = [];
        ...
        legPositions.forEach((position, index) => {
            ...
            switch (culturalData.styleElements.legStyle) {
                case 'tapered':
                    legGeometry = new THREE.CylinderGeometry(dimensions.legThickness * 0.6, dimensions.legThickness * 0.8, dimensions.seatHeight, 8);
                    break;
                case 'cylindrical':
                    legGeometry = new THREE.CylinderGeometry(dimensions.legThickness / 2, dimensions.legThickness / 2, dimensions.seatHeight, 8);
                    break;
                case 'turned':
                ...
                case 'straight':
                default:
                    legGeometry = new THREE.BoxGeometry(dimensions.legThickness, dimensions.seatHeight, dimensions.legThickness);
            }
    ...
    generateArmrests(dimensions, params) {
        const armrests = [];
        ...
        leftArmrest.position.set(-(dimensions.seatWidth / 2) - 0.04, armrestHeight, 0);
    ...
    addCulturalDetails(dimensions, params) {
        const details = [];
        const culturalData = this.culturalData.get(params.culture);
    ...
    createFrenchPaddedSeat(dimensions) {
    ...
    createBergereBackrest(dimensions) {
    ...
    createCabrioleLeg(dimensions) {
        ...
        for (let i = 0; i <= 24; i++) {
            ...
            if (i > 3 && i < 8)
                radius *= 1.15; // Upper bulge
            if (i > 16 && i < 20)
                radius *= 0.85; // Lower taper
            if (i > 20)
                radius *= 1.05; // Foot
    ...
    createFrenchArmrestGeometry(width) {
    ...
    createFrenchSavoirVivreDetails(dimensions, params) {
        const details = [];
        // Add button tufting
    ...
    createContouredSeat(dimensions) {
        const shape = new THREE.Shape();
        const width = dimensions.seatWidth;
        const depth = dimensions.seatDepth;
        const radius = 0.02;
    ...
    addSafetyRounding(geometry) {
        geometry.userData.safetyRounding = true;
        return geometry;
    }
    createBambooAccents(dimensions) {
        const accents = [];
    ...
    createJoineryDetails(dimensions) {
        const details = [];
    ...
    createCarvedDetails(dimensions) {
        return [];
    }
    createCushionElements(dimensions) {
        return [];
    }
    createCurvedBackrest(dimensions) {
        return new THREE.BoxGeometry(dimensions.seatWidth * 0.9, dimensions.backrestHeight, dimensions.backrestThickness);
    }
    createErgonomicBackrest(dimensions) {
        return new THREE.BoxGeometry(dimensions.seatWidth * 0.9, dimensions.backrestHeight, dimensions.backrestThickness);
    }
    createPaddedSeat(dimensions) {
        return new THREE.BoxGeometry(dimensions.seatWidth, dimensions.seatThickness * 1.5, dimensions.seatDepth);
    }
    applyMaterials(chair, params) {
    ...
    createWoodMaterial(woodType, colorPalette) {
        const woodColors = {
            'wood-oak': '#D2B48C',
            'wood-pine': '#F5DEB3',
            'wood-cherry': '#8B4513',
    ...
    createUpholsteryMaterial(upholsteryType, colorPalette) {
...
export function createEnhancedTestChair() {
    const template = new EnhancedChairTemplate();
    const frenchChairParams = {
        culture: 'french',
        targetAge: 'adult',
...
//# sourceMappingURL=EnhancedChairTemplate.js.map