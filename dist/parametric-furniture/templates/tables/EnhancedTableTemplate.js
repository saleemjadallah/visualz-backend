...
export class EnhancedTableTemplate {
    constructor() {
    ...
    mapLegConfiguration(legStyle, legConfiguration) {
        if (legStyle)
            return legStyle;
    ...
    convertToTableParameters(params) {
        return {
            ...params,
            tableType: params.type.includes('table') ?
                params.type.replace('-table', '') : 'dining',
    ...
    validateCulturalParameters(params, dimensions) {
    ...
    generateTabletop(dimensions, params) {
        const culturalData = this.culturalData.get(params.culture);
        let topGeometry;
    ...
    createRectangularTop(dimensions, params) {
        const culturalData = this.culturalData.get(params.culture);
        let geometry;
        switch (culturalData.styleElements.edgeProfile) {
            ...
            case 'square':
            default:
                geometry = new THREE.BoxGeometry(dimensions.width, dimensions.topThickness, dimensions.depth);
        }
        return geometry;
    }
    createFrenchElegantOval(dimensions, params) {
        // Create sophisticated French oval with refined proportions
        const curve = new THREE.EllipseCurve(0, 0, // center
    ...
    createFrenchMoldedTop(dimensions) {
    ...
    createRoundedRectangularTop(dimensions) {
        const shape = new THREE.Shape();
        const width = dimensions.width;
        const depth = dimensions.depth;
    ...
    createBeveledRectangularTop(dimensions) {
        const geometry = new THREE.BoxGeometry(dimensions.width, dimensions.topThickness, dimensions.depth);
        geometry.userData.bevelStyle = 'italian-elegant';
        return geometry;
    }
    createLiveEdgeTop(dimensions) {
        const geometry = new THREE.BoxGeometry(dimensions.width, dimensions.topThickness, dimensions.depth);
        geometry.userData.edgeStyle = 'live-edge';
        return geometry;
    }
    createOvalTop(dimensions, params) {
        const curve = new THREE.EllipseCurve(0, 0, dimensions.width / 2, dimensions.depth / 2, 0, 2 * Math.PI, false, 0);
    ...
    createRoundTop(dimensions, params) {
    ...
    generateTableBase(dimensions, params) {
    ...
    createFourLegBase(dimensions, params) {
        const legs = [];
        ...
        positions.forEach((position, index) => {
            ...
            switch (culturalData.styleElements.supportStyle) {
                case 'minimal':
                    legGeometry = new THREE.BoxGeometry(dimensions.legThickness * 0.8, legHeight, dimensions.legThickness * 0.8);
                    break;
                case 'substantial':
                    legGeometry = new THREE.CylinderGeometry(dimensions.legThickness / 2, dimensions.legThickness / 2, legHeight, 8);
                    break;
                case 'decorative':
                ...
                case 'salon-elegant':
                    ...
                    break;
                default:
                    legGeometry = new THREE.BoxGeometry(dimensions.legThickness, legHeight, dimensions.legThickness);
            }
    ...
    createFrenchSalonBase(dimensions, params) {
    ...
    createCabrioleTableBase(dimensions, params) {
    ...
    createFrenchSalonLeg(dimensions, height) {
        ...
        for (let i = 0; i <= 24; i++) {
            ...
            if (i > 2 && i < 5)
                radius *= 1.08; // Subtle capital
            if (i > 19 && i < 22)
                radius *= 0.92; // Gentle taper toward foot
            if (i > 22)
                radius *= 1.05; // Slight foot
            points.push(new THREE.Vector2(radius, y));
        }
        return new THREE.LatheGeometry(points, 16);
    }
    createCabrioleTableLeg(dimensions, height) {
        ...
        for (let i = 0; i <= 28; i++) {
            ...
            if (i > 3 && i < 8)
                radius *= 1.12; // Upper bulge
            if (i > 8 && i < 16)
                radius *= 0.95; // Curve inward
            if (i > 16 && i < 20)
                radius *= 0.88; // Continue taper
            if (i > 20 && i < 24)
                radius *= 1.06; // Lower bulge
            if (i > 24)
                radius *= 1.08; // Foot
            points.push(new THREE.Vector2(radius, y));
        }
    ...
    createPedestalBase(dimensions, params) {
        ...
        const pedestalGeometry = new THREE.CylinderGeometry(baseRadius * 0.6, baseRadius, baseHeight, 12);
    ...
    createTrestleBase(dimensions, params) {
        const trestleComponents = [];
        const baseHeight = dimensions.height - dimensions.topThickness;
        ...
        const beamGeometry = new THREE.BoxGeometry(dimensions.width * 0.6, supportWidth, supportWidth);
    ...
    createTrestleSupport(width, height, depth) {
    ...
    createDecorativeLeg(dimensions, height) {
        const points = [];
        const maxRadius = dimensions.legThickness / 2;
        for (let i = 0; i <= 20; i++) {
            const y = (i / 20) * height;
    ...
    calculateLegPositions(dimensions) {
    ...
    generateSupports(dimensions, params) {
        const supports = [];
    ...
    createTableAprons(dimensions, params) {
        const aprons = [];
        ...
        const frontBackApronGeometry = new THREE.BoxGeometry(dimensions.width - (dimensions.overhang * 2), apronHeight, apronThickness);
    ...
    generateStorage(dimensions, params) {
        const storage = [];
        if (params.includeStorage) {
            const drawerGeometry = new THREE.BoxGeometry(dimensions.width * 0.6, 0.1, dimensions.depth * 0.4);
    ...
    addCulturalTableElements(dimensions, params) {
    ...
    createFrenchSavoirVivreTableElements(dimensions, params) {
        const elements = [];
    ...
    createTeaCeremonyElements(dimensions) {
        const elements = [];
    ...
    createJapaneseAccents(dimensions) {
        return [];
    }
    createScandinavianAccents(dimensions) {
        return [];
    }
    createItalianAccents(dimensions) {
        return [];
    }
    applyTableMaterials(table, params) {
    ...
    createTopMaterial(topMaterial, colorPalette) {
    ...
    createBaseMaterial(baseMaterial, colorPalette) {
        const materialColors = {
            'wood-oak': '#D2B48C',
            'wood-cherry': '#8B4513',
            'wood-walnut': '#654321',
    ...
    createStorageMaterial(material, colorPalette) {
        return this.createBaseMaterial(material, colorPalette);
    }
    createAccentMaterial(culture, colorPalette) {
    ...
    calculateTableAuthenticity(params) {
        const culturalData = this.culturalData.get(params.culture);
        let score = 100;
    ...
    getCulturalSignificance(culture, tableType) {
        const significance = {
    ...
    getMaintenanceInstructions(params) {
        const instructions = [
...
export function createEnhancedTestTable() {
    const template = new EnhancedTableTemplate();
    const frenchDiningTableParams = {
        tableType: 'dining',
        culture: 'french',
        capacity: 8,
...
//# sourceMappingURL=EnhancedTableTemplate.js.map