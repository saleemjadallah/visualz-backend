...
export class AIParameterAnalyzer {
    ...
    async analyzeUserRequirements(userInput) {
    ...
    buildAnalysisPrompt(userInput, culturalContext) {
...
Your responses must be technically precise, culturally sensitive, and practically implementable. Always output valid JSON.
    ...
    async optimizeParameters(baseParameters, constraints) {
...
Return the optimized parameters in the same JSON format as a ParametricParameters object.
    ...
    validateAndSanitizeAIResponse(aiAnalysis, userInput) {
        // Validate structure
        if (!aiAnalysis.furniture_pieces || !Array.isArray(aiAnalysis.furniture_pieces)) {
            return this.fallbackAnalysis(userInput);
        }
        // Sanitize furniture pieces
        const sanitizedPieces = aiAnalysis.furniture_pieces.map((piece) => {
            return {
    ...
    sanitizeParameters(params, userInput) {
    ...
    validateParametricParameters(params) {
        return params &&
...
//# sourceMappingURL=AIParameterAnalyzer.js.map