...
class SimpleAIThreeJSIntegrationService {
    ...
    async generateThreeJSScene(mockResponse) {
        ...
        if (mockResponse.furniture_specifications.some((spec) => spec.type === 'balloon-system')) {
        ...
        if (mockResponse.furniture_specifications.some((spec) => spec.type === 'photo-backdrop')) {
        ...
        if (mockResponse.furniture_specifications.some((spec) => spec.type === 'celebratory-props')) {
        ...
        if (mockResponse.furniture_specifications.some((spec) => spec.type === 'interactive-prop')) {
        ...
        if (mockResponse.furniture_specifications.some((spec) => spec.type === 'gift-display')) {
        ...
        if (mockResponse.furniture_specifications.some((spec) => spec.type === 'ceremonial-element')) {
    ...
    createCakeDisplay() {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            const radius = 0.15;
            candle.position.set(Math.cos(angle) * radius, 1.04, Math.sin(angle) * radius);
